...
This generator relies on four parameters: A _multiplier_ `a`, an _increment_ `c`, a _modulus_ `m`, and an _starting value_ `seed`. It uses a chained operation -- every time we generate a new value, we use the previous value as the seed. See the Wikipedia page for more details.
:::
...
- Your total number of samples is bounded and not immense -- things get tricker if you need to exceed about 100 samples.
...
::: {.aside title="Why so many events?"}
##### Why so many events?
...
* [Data Dictionary CSV](assets/data_dictionary.csv) - For **Designer** > **Upload** or **API**
* [Events CSV](assets/events.csv) - For **Project Setup** > **Define My Events**
* [Event Mappings CSV](assets/event_mappings.csv) - For **Project Setup** > **Designate Instruments for My Events**
* [ASI List CSV](assets/asi_list.csv) - For importing Automated Survey Invitations
...
* **Similar seeds:** Similar seed values produce similar early LCG values. The processed `seed` field runs the LCG a few times to mitigate this.
