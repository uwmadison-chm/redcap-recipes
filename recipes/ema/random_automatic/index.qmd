...
1. **Time window mapping:** Each sampling period (e.g., "morning slot") has a start time and duration. The generated numbers are used to pick a time within each period.
1. **Event-per-sample architecture:** Each potential sample gets its own event with an ASI that triggers at the calculated time.
