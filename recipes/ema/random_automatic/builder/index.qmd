...
<div>
<label for="day-jitter" class="form-label">Day jitter (± minutes)</label>
<input type="number" class="form-control" id="day-jitter" value="15" min="0" max="60" step="5" style="width: 120px;">
</div>
</div>
...
For example, with ±15 minute jitter, each day's windows shift by a random amount between -15 and +15 minutes. Over many days, this provides broader coverage of the waking period.
...
Define the time windows during which samples can be scheduled. Each window produces one sample per day, randomly placed within that window.
...
<button id="add-window-btn" class="btn btn-outline-primary me-2">Add Window</button>
