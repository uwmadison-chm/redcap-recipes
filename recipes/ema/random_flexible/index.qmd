...
* **Scheduled Alerts don't appear in the log immediately:** Between the time an Alert is scheduled and when it's actually sent, it will not appear in the **Alerts Log**. This is normal behavior — don't panic when troubleshooting!
* **Checking message delivery:** The only way to check whether messages have been delivered is to use the **Notification Log** in **Alerts & Notifications**. It's important to download the log on a regular basis and reconcile it against the schedule to make sure your messages are going out. You'll need a script to do that reconciliation, unfortunately.
